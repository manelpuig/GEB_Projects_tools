...
def Robot_online(RDK: Robolink, robot: Item, online: bool):
    ...
    if RUN_ON_ROBOT:
        # Configure connection parameters for the RoboDK driver
        robot.setConnectionParams(ROBOT_IP, ROBOT_PORT, "/", ROBOT_USER, ROBOT_PASS)

        # Try to connect
        time.sleep(1.0)
        success = robot.ConnectSafe(ROBOT_IP)  # Try to connect once
        time.sleep(0.5)
        ...
        # Run API programs on the real robot (and the simulator as a digital twin)
        RDK.setRunMode(RUNMODE_RUN_ROBOT)
        print("✅ Connection to UR Successful! RUNMODE_RUN_ROBOT enabled.")
    else:
        # Offline: run everything in simulation
...
def control_thread(robot, base, gripper, needle,
    ...
    while not stop_flag:
        ...
        # -------------------------
        # 2) Apply Gripper IMU (world -> relative)
        # -------------------------
...
def main():
    ...
    # Select run mode (simulation or real robot) + connect if needed
    Robot_online(RDK, robot, RUN_ON_ROBOT)

    # Save initial poses to restore on exit (best-effort)
